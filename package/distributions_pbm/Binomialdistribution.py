...
class Binomial(Distribution):
    ...
    def pdf(self, k):
        ...
        mid_result = math.factorial(self.n)/(math.factorial(k)*math.factorial(self.n - k))        
        return (mid_result * ((self.p)**k)*((1-self.p)**(self.n - k)))
    ...
    def plot_bar_pdf(self):
        ...
        for k in range(0, n+1):
            pdf(k)
            x_vals.append(k)
            y_vals.append(self.p)
        plt.bar(x = [range(0, n+1)], height = [(1 - self.p) * self.n, self.p * self.n])
        plt.title('Bar Chart of Data')
        plt.xlabel('outcome')
        plt.ylabel('count') 
    ...
    def __add__(self, other):
        ...
        new_binomial = Binomial(new_p, new_n)       
        return new_binomial
